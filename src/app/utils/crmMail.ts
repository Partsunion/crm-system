...
export const previewCrmMail=(leadId:string,subject:string,body:string)=>request<CrmMailDraft>('/leads/'+encodeURIComponent(leadId)+'/preview',{subject,body});
export const sendCrmMail=(leadId:string,input:{requestId:string;subject:string;body:string})=>request<{success:boolean;recipient:string;sender:string;messageId:string}>('/leads/'+encodeURIComponent(leadId)+'/send',input);
