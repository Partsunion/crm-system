...
import {Mail,Send,Loader2,Eye,PenLine,RefreshCw,Sparkles,Undo2,Paperclip} from 'lucide-react';
...
export function LeadEmailComposer({leadId,conversationNotes='',onClose,onSent,onState}:{leadId:string;conversationNotes?:string;onClose:()=>void;onSent:()=>void;onState:(state:{dirty:boolean;busy:boolean})=>void}){
 ...
 const dirty=Boolean(draft && (subject!==draft.subject || body!==draft.body || includeBrochure));
 useEffect(()=>{onState({dirty,busy:sending||generating||previewing});return()=>onState({dirty:false,busy:false});},[dirty,sending,generating,previewing,onState]);
 useEffect(()=>{
  let alive=true;
 ...
 const close=()=>{if(sending||generating||previewing)return;if(dirty && !confirm('E-Mail-Entwurf verwerfen?'))return;onClose();};
 async function generate(){
  if(generationLock.current||attempted||sending||previewing)return;
  generationLock.current=true;setGenerating(true);setError('');
  const controller=new AbortController();generation.current=controller;
  try{const result=await personalizeCrmMail(leadId,{conversationNotes,instructions,subject,body,includeBrochure},controller.signal);if(controller.signal.aborted)return;setPrevious({subject,body});setSubject(result.subject);setBody(result.body);setHtml(result.html||'');setPreview(false);}
  catch(e){if(!controller.signal.aborted)setError((e as Error).message);}
  finally{generationLock.current=false;if(!controller.signal.aborted)setGenerating(false);}
 }
 async function showPreview(){setError('');setPreviewing(true);try{const d=await previewCrmMail(leadId,subject,body,includeBrochure);setHtml(d.html||'');setPreview(true);}catch(e){setError((e as Error).message);}finally{setPreviewing(false);}}
 async function toggleBrochure(enabled:boolean){
  if(attempted||generating||previewing||sending)return;
  setPreviewing(true);setError('');
  try{const d=await previewCrmMail(leadId,subject,body,enabled);setIncludeBrochure(enabled);setBody(d.body);setHtml(d.html||'');setPrevious(null);setDraft(previous=>previous?{...previous,brochure:d.brochure}:previous);}
  catch(e){setError((e as Error).message);}
  finally{setPreviewing(false);}
 }
 async function send(){
  if(sendLock.current || generating || previewing || !draft?.connected || !subject.trim() || !body.trim() || uncertain)return;
  sendLock.current=true;setSending(true);setAttempted(true);setError('');
  try{const result=await sendCrmMail(leadId,{requestId:requestId.current,subject,body,includeBrochure});toast.success(`E-Mail von ${result.sender} an ${result.recipient} verschickt.`);onSent();onClose();}
 ...
 return <Modal title="E-Mail an den Lead" subtitle="Nach dem Gespräch · persönlich von dir" size="lg" onClose={close} footer={<div className="flex w-full flex-wrap items-center justify-between gap-2"><Button variant="ghost" onClick={close} disabled={sending||generating||previewing}>Schließen</Button><div className="flex gap-2"><Button variant="outline" onClick={()=>preview?setPreview(false):void showPreview()} disabled={loading||sending||generating||previewing||!draft?.connected||!subject.trim()||!body.trim()}>{previewing?<Loader2 className="size-4 animate-spin"/>:preview?<PenLine className="size-4"/>:<Eye className="size-4"/>}{preview?'Bearbeiten':'Vorschau'}</Button><Button onClick={()=>void send()} disabled={loading||sending||generating||previewing||uncertain||!draft?.connected||!recipientValid||!subject.trim()||!body.trim()}>{sending?<Loader2 className="size-4 animate-spin"/>:attempted?<RefreshCw className="size-4"/>:<Send className="size-4"/>}{sending?'Wird versendet …':attempted?'Versand prüfen / fortsetzen':'E-Mail senden'}</Button></div></div>}>
  {loading?<div className="flex items-center gap-2 py-8 text-sm text-text-muted"><Loader2 className="size-4 animate-spin"/>Vorlage wird geladen …</div>:<div className="space-y-4">
   ...
   {!preview&&<div className="space-y-2 border-b border-border-subtle pb-4">
    ...
    <input id="mail-focus" value={instructions} maxLength={1000} disabled={attempted||generating||previewing} onChange={e=>setInstructions(e.target.value)} placeholder="Optional: z. B. Lagerbestand ansprechen, kurz halten …" className={inputSized}/>
    <div className="flex flex-wrap items-center gap-3"><Button variant="ghost" size="sm" disabled={attempted||generating||previewing||!draft?.aiAvailable||!draft?.sender} onClick={()=>void generate()}>{generating?<Loader2 className="size-3.5 animate-spin"/>:<Sparkles className="size-3.5"/>}{generating?'Text wird personalisiert …':'Neu formulieren'}</Button>{previous&&!attempted&&!generating&&!previewing&&<button type="button" className="inline-flex items-center gap-1 text-xs text-text-muted" onClick={()=>{setSubject(previous.subject);setBody(previous.body);setPrevious(null);setPreview(false);}}><Undo2 className="size-3.5"/>Vorherigen Text wiederherstellen</button>}</div>
   ...
   <div className="border-b border-border-subtle pb-3 text-sm">
    <label className="inline-flex cursor-pointer items-center gap-2"><input type="checkbox" checked={includeBrochure} disabled={attempted||generating||previewing||sending} onChange={e=>void toggleBrochure(e.target.checked)} className="size-4 accent-primary"/><span>Mit Broschüre versenden</span>{previewing&&<Loader2 className="size-3.5 animate-spin"/>}</label>
    {includeBrochure&&draft?.brochure?<p className="mt-2 flex items-center gap-2 break-all text-xs text-text-secondary"><Paperclip className="size-3.5 shrink-0"/>{draft.brochure.filename}</p>:<p className="mt-1 text-xs text-text-muted">Optional · passende Broschüre zum Händlertyp. Der Hinweis im Text wird automatisch angepasst.</p>}
   </div>
