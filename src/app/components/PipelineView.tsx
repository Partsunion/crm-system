...
import { getLeads, saveLead, deleteLead, getSettings, getCurrentUser, type Lead, type PipelineStage } from '../utils/storage';
...
export function PipelineView({ onOpenCalendar }: { onOpenCalendar?: (lead: Lead) => void } = {}) {
  ...
  const ownAliases = useMemo(() => new Set(
    [currentUser?.id, currentUser?.username, currentUser?.name]
      .filter(Boolean)
      .map((value) => value!.trim().toLocaleLowerCase('de')),
  ), [currentUser?.id, currentUser?.username, currentUser?.name]);
  const ownerFilteredLeads = useMemo(() => assignee === 'mine'
    ? searchedLeads.filter((lead) => ownAliases.has((lead.assignedTo || '').trim().toLocaleLowerCase('de')))
    : searchedLeads, [searchedLeads, assignee, ownAliases]);
  const workspace = useMemo(() => pipelineWorkspace(ownerFilteredLeads, stages, assignee === 'mine' ? 'all' : assignee, focus, today, workspaceTime), [ownerFilteredLeads, stages, assignee, focus, today, workspaceTime]);
