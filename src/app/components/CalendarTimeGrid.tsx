...
export function CalendarTimeGrid({ days, byDay, today, onOpen, onCreate }: { days: Date[]; byDay: Record<string, Appointment[]>; today: string; onOpen: (appointment: Appointment) => void; onCreate: (day: string, time: string) => void }) {
  ...
  return <>{days.length > 1 && <p className="shrink-0 border-b border-border-subtle bg-elevated/40 px-3 py-2 text-xs text-text-muted xl:hidden">Zeitraster horizontal verschieben, um weitere Tage zu sehen.</p>}<div className="min-h-0 flex-1 overflow-auto" aria-label="Kalender mit Stundenraster" data-calendar-scroll><div style={{ minWidth: days.length > 1 ? 850 : 300 }}>
