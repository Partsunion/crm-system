...
describe('calendar appointment editor', () => {
  ...
  it('keeps the day panel beside the month only and does not stack it below week view', async () => {
    render(<KalenderView />);
    await waitFor(() => expect(screen.getByText('Kalender-Zeitachse')).toBeInTheDocument());
    expect(screen.queryByLabelText('Ausgewählter Tag')).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'Monat' }));
    expect(screen.getByLabelText('Ausgewählter Tag')).toBeInTheDocument();
    fireEvent.click(screen.getByRole('button', { name: 'Woche' }));
    expect(screen.queryByLabelText('Ausgewählter Tag')).not.toBeInTheDocument();
    expect(screen.getByText('Kalender-Zeitachse')).toBeInTheDocument();
  });
});
