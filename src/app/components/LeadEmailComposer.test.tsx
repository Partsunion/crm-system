...
it('personalizes on opening from current conversation notes and keeps the result editable',async()=>{
 ...
 fireEvent.change(screen.getByLabelText('Nachricht'),{target:{value:'Mein ergänzter Text'}});fireEvent.click(screen.getByRole('button',{name:'Vorschau'}));await screen.findByTitle('Vorschau der Partsunion-E-Mail');expect(api.preview).toHaveBeenCalledWith('lead-1','Für Ihr Gebrauchtteilelager','Mein ergänzter Text');expect(api.send).not.toHaveBeenCalled();
});
