...
import {
  ...
  getLeadLists, createLeadList, addLeadsToList, removeLeadsFromList, type LeadList,
...
const hasSource = (lead: Lead, pattern: RegExp): boolean =>
  [lead.leadSource, lead.source].some((value) => pattern.test((value || '').trim()));
export const SOURCE_SEGMENTS: { key: string; label: string; match: (l: Lead) => boolean }[] = [
  { key: 'all', label: 'Alle', match: () => true },
  { key: 'src:scraper', label: 'Scraping', match: (l) => hasSource(l, /^(scraper|scraping|umkreissuche|osm)$/i) },
  { key: 'src:website', label: 'Website', match: (l) => hasSource(l, /^(website|webseite|website[_ -]?(lead|formular|form)|contact[_ -]?form)$/i) },
  { key: 'src:google', label: 'GoogleAds', match: (l) => hasSource(l, /^google[_ -]?(ads?|adwords|lead)$/i) },
  { key: 'src:meta', label: 'MetaAds', match: (l) => hasSource(l, /^(meta([_ -]?(ads?|lead(s)?))?|(facebook|instagram)[_ -]?(ads?|lead(s)?))$/i) },
  { key: 'src:manual', label: 'Manuell', match: (l) => hasSource(l, /^(manuell|manual)$/i) },
];
...
export function LeadsView({
  ...
  // Frühere persönliche Listenfilter dürfen nach der vereinfachten
  // Quellenleiste nicht unsichtbar weiterfiltern.
  useEffect(() => {
    if (!SOURCE_SEGMENTS.some((segment) => segment.key === activeSeg)) setActiveSeg('all');
  }, [activeSeg, setActiveSeg]);
        ...
        <div className="crm-lead-toolbar flex flex-wrap items-center gap-2 px-3 py-2.5">
          ...
          <Popover open={filtersOpen} onOpenChange={setFiltersOpen}>
            ...
            <PopoverContent id="lead-filters" aria-label="Weitere Leadfilter" align="end" className="crm-filter-popover max-h-[min(70vh,520px)] w-[min(620px,calc(100vw-24px))] space-y-4 overflow-y-auto border-border-subtle bg-surface p-4 text-text-primary shadow-modal">
              ...
              <div><p className="mb-2 text-xs font-medium text-text-secondary">Quelle</p><SegmentBar active={activeSeg} onSelect={key=>{setActiveSeg(key);clearSelection();}} segCount={segCount}/></div>
